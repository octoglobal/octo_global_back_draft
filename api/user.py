    ...
    if request.method == "GET":
        ...
        try:
            ...
            search_shops_startswith = Shop.select(Shop.title, Shop.url)\
                .where(Shop.title.startswith(search_string)).limit(search_results_limit)\
                .order_by(Shop.rating.desc(), Shop.title)
            search_shops_contains = Shop.select(Shop.title, Shop.url)\
                ...
                .limit(search_results_limit - len(search_shops_startswith)).order_by(Shop.rating.desc(), Shop.title)
        ...
        if len(db_tags) > 0:
            shops = Shop.select().offset(offset).limit(page_limit).where(Tag_of_shops.tag_id << db_tags)\
                ...
                .order_by(Shop.rating.desc(), Shop.title).group_by(Shop.id)
        else:
            shops = Shop.select().offset(offset).limit(page_limit).order_by(Shop.rating.desc(), Shop.title)
        shops_tags = Tag_of_shops.select(Tag_of_shops.shop_id, Tag_of_shops.tag_id, Tag.title)\
